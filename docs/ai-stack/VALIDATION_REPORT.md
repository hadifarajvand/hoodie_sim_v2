...
Bootstrap workflow layer created and validated. OpenCode, Graphify, and RuFlo are all present locally. Graphify generated a project graph. RuFlo was initialized, memory was seeded, and the runtime config was aligned to `hierarchical-mesh` with 8 agents.
...
No project-local OpenCode MCP schema was verified. The RuFlo-generated `.mcp.json` exists, but OpenCode MCP wiring was intentionally left as policy/documentation rather than a guessed schema edit.
Noninteractive CLI verification of OpenCode’s resolved config was blocked by the sandboxed log path and interactive behavior of `opencode debug` / `opencode agent list`.
`opencode run --help` and `opencode mcp --help` confirmed those CLI surfaces exist, but `opencode debug config`, `opencode mcp list`, and agent listing were not reliably enumerable in this sandbox.
