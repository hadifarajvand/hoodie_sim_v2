...
| `tests/unit/test_reward_equation_terminal_reward_contract_sign.py` | `test_reward_for_terminal_task_signs` | `reward_for_terminal_task(completed)` returns `-3.0` for the fixture | Fails because runtime reward uses `completion_slot - arrival_slot + 1` | delayed reward / reward equation | obsolete | Encodes an older reward expectation that conflicts with the accepted delayed terminal reward contract | Update the obsolete expectation in Phase 3B |
