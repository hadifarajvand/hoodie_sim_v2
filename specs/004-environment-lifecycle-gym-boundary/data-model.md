...
- **Fields**: slot, queue load, active task ID, source agent ID, task features, legal-action mask, topology hint, load hints, history length, edge-agent keyed mapping
...
## Helper-only SlotEngine boundary

- `SlotEngine` is a helper container for queue-admission and offload-completion helpers only.
- It does not own lifecycle sequencing, slot progression, or observation construction.
- Any lifecycle-ordering state remains owned by `HoodieGymEnvironment`.
