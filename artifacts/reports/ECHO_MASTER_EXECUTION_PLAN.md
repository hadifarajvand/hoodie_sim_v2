...
| Google Drive current revision | `280` |
...
The plan cannot contain its own future commit SHA. The controller therefore discovers the approved v4.3 plan commit as the unique first descendant of the Planning-base HEAD whose diff changes only `artifacts/reports/ECHO_MASTER_EXECUTION_PLAN.md` and whose content declares `ECHO-MEP-v4.3`. Before the first executor, later bootstrap commits may change only `scripts/control/validate_echo_plan.py` and G3 control files. The controller stores the discovered approved plan commit in `EXECUTION_STATE.json`; all later work must descend from it.
...
Revision 280 is a **material method change**, not metadata-only. It supersedes the former fixed 30-destination/32-output architecture. The current authoritative scalability contract is:
...
- each EA in an ECHO run owns an independent learner, replay buffer, online network, target network, optimizer, exploration state, forecasting state, and checkpoint; source-specific interval accumulators, task ownership, and event provenance remain separate;
...
- Equation (58) with selected-action lateness term plus Equations (55)–(57) reward terms.
- Equation (59) as one discounted source-EA interval between consecutive actual decisions.
- no overlapping task-level replay transitions; next-mask, terminal state, terminal flag, final sojourn, and zero continuation are explicit.
- target discount `γ^Δ_(n,m)`.
- masked Dueling Double-DQL from Equations (61)–(67).
- independent per-EA ECHO learners plus separate source-specific event ownership.
...
| Existing ECHO agent architecture must prove independent per-EA learners and checkpoint lineage | `ECHO-015` |
| Event-SMDP exists but boundary ownership, next-mask, and terminal semantics remain unproven | `ECHO-014` and `ECHO-018` |
...
Before any executor:
1. discover and store the approved v4.3 plan commit using Section 1.1.
...
5. keep `NEXT_TASK.json` null until reconciliation review and merge; only then may the controller authorize `SRC-001`, attempt 3, branch `task/src-001-r3`.
...
| ECHO-015 | BLOCKED | ECHO-014 | Implement Equations (61)–(67) masked DDQL with independent per-EA learners |
...
- **Ordered operations:**
  ...
  4. Verify Equations 1–67, Algorithm 1 lines 1–23, Algorithm 2 lines 1–14.
  5. Create 69 audit rows and map every non-MATCH row to one task and one exact test path.
  6. Explicitly classify the revision-280 scalability change: N+2 size-specific action/state dimensions and parameter sharing among EAs within one run.
- **Exact command:**
```bash
...
- **Required reads:** revision-280 source section; `src/echo_action_space.py`; `src/echo_types.py`; topology.
...
- **Required reads:** live equations and revision-280 scalability paragraph; EVAL-001 schemas; state/history/status modules.
...
### ECHO-015 — Implement Equations (61)–(67) masked DDQL with independent per-EA learners
...
- **Acceptance:**
  ...
  - independent-per-EA behavior verified.
...
- **Ordered operations:**
  ...
  2. enforce checkpoint N compatibility and independent per-EA learners metadata.
- **Exact command:**
```bash
...
- **Ordered operations:**
  1. Document equation/algorithm map, chronology, N+2 size-specific architecture, independent per-EA learners, state/mask/reward/SMDP/DDQL/LSTM/isolation/tests/limits.
- **Exact command:**
```bash
python3 - <<'PY'
from pathlib import Path
...
- **Acceptance:**
  ...
  - revision-280 architecture explicit.
...
- for each tested N, ECHO uses independently trained N-specific checkpoints with N+2 action outputs and independent per-EA learners.

## 10. Exact executor prompt

```text
You are the single-task executor for hadifarajvand/hoodie_sim_v2.
The sole authority is artifacts/reports/ECHO_MASTER_EXECUTION_PLAN.md version ECHO-MEP-v4.3 and artifacts/control/NEXT_TASK.json.
...
Use artifacts/reports/ECHO_MASTER_EXECUTION_PLAN.md version ECHO-MEP-v4.3 and G2/G3 as the sole control authority.
In BOOTSTRAP mode, discover the approved plan commit by the Section 1.1 rule, run the structural preflight, initialize G3, record SRC-001 as pending independent review, commit control files, print the controller state, and stop.
...
- ECHO-009/012/015 implement revision-280 N+2, N-specific checkpoints, and independent per-EA learners.
...
After bootstrap, `NEXT_TASK.json` remains null until reconciliation review and merge. The controller may authorize only `SRC-001` attempt 3 from the resulting main control commit. `BASE-001` and all ECHO implementation remain unauthorized until `SRC-001` completes the 69-row audit and is independently verified. The entire plan then advances one reviewed and merged card at a time through `HANDOFF-001`. Permanent deletion is prohibited.
