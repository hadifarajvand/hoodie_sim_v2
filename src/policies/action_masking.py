...
def _horizontal_destination_is_legal(
    context: PolicyContext, action: str
) -> bool:
    destination = action.removeprefix("horizontal_")
    source = context.observation.get("source_agent_id")
    raw_topology = context.observation.get("topology", ())
    topology = (
        {str(value) for value in raw_topology}
        if isinstance(raw_topology, (list, tuple, set))
        else set()
    )
    return (
        bool(destination)
        and (source is None or destination != str(source))
        and destination in topology
    )
...
def select_legal_action(context: PolicyContext, action: str) -> str:
    if action in ("local", "compute_local"):
        if not (
            context.legal_action_mask.get("local", False)
            or context.legal_action_mask.get("compute_local", False)
        ):
        ...
        if not (
            context.legal_action_mask.get("vertical", False)
            or context.legal_action_mask.get("offload_vertical", False)
            or context.legal_action_mask.get("cloud", False)
        ):
            raise ValueError(f"Illegal action: {action}")
