...
class HoodieProposedMethodRunner:
    def build(self):
        return build_feature_080_report()

    def render(self) -> str:
        return render_feature_080_report(self.build())

    def write(self, output_dir: Path | str = DEFAULT_OUTPUT_DIR) -> Path:
        return write_feature_080_report(output_dir)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the HOODIE proposed-method implementation report.")
    parser.add_argument("--output-dir", default=None, help="Optional directory for markdown/json output.")
    args = parser.parse_args(argv)
    runner = HoodieProposedMethodRunner()
    if args.output_dir:
