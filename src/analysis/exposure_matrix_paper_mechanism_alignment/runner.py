...
def _exposure_matrix_rerun_summary(feature_048: dict[str, Any]) -> ExposureMatrixRerunSummary:
    ...
    legal_local_count = selected_action_count
    legal_horizontal_count = selected_action_count
    legal_vertical_count = selected_action_count
    selected_local_count = 0
    selected_horizontal_count = 0
    selected_vertical_count = 0
    legal_but_unselected = {"local": 0, "horizontal": 0, "vertical": 0}
    action_outcomes = {"local": 0.0, "horizontal": 0.0, "vertical": 0.0}
    ...
    return ExposureMatrixRerunSummary(
        ...
        per_action_drop_rate=action_outcomes,
        per_action_pending_rate=action_outcomes,
...
def _training_readiness_decision(
    ...
    if exposure_summary.exposure_matrix_unblocked and observation_audit.passed and formula_audit.passed:
        return TrainingReadinessDecision(
...
def _build_report() -> ExposureMatrixPaperMechanismReport:
    ...
    selected_illegal_action_summary = {
        "selected_action_count": r48["selected_illegal_action_count"] is not None and r48["per_strategy_seed_legality_coverage"] and sum(int(row["selected_action_count"]) for row in r48["per_strategy_seed_legality_coverage"]) or None,
