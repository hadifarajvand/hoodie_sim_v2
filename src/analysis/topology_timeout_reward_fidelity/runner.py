...
def validate_scope(paths: Sequence[str] | None = None) -> list[str]:
    ...
    if forbidden:
        raise RuntimeError("Dirty or diff paths outside approved Feature 070 scope: " + ", ".join(sorted(set(forbidden))))
    return checked
...
def build_report() -> Any:
    ...
    validate_scope()
    return build_feature_070_report(changed_files=changed_files)


def run(output_dir: Path | str | None = None) -> Any:
    report = build_report()
    if output_dir is not None:
        return report, write_feature_070_report(report, output_dir)
    return report
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if argv and "--json" in argv:
        ...
        print(render_feature_070_report(report))
    return 0
