...
from .config import DEADLINE_PRESSURE_LEVELS, EvaluationConfig, RUNTIME_MODE_PAPER, TOPOLOGY_MODE_PAPER_FIGURE_7, WORKLOAD_TASK_COUNTS, WORKLOAD_LEVELS, WORKLOAD_SCENARIO_DURATION
from .model import ScenarioContext, TaskBlueprint


_DEADLINE_OFFSET = {"relaxed": 2, "moderate": 0, "tight": -1}
_WORKLOAD_DEADLINE_SHIFT = {"low": 1, "medium": 0, "high": -1}
...
_SCENARIOS: dict[str, ScenarioBlueprint] = {
    ...
    "legal_horizontal_offload": ScenarioBlueprint("legal_horizontal_offload", 3, 2, 3, True, ("6",), ("2",), "horizontal_ready"),
    "illegal_horizontal_destination_attempt": ScenarioBlueprint("illegal_horizontal_destination_attempt", 2, 2, 2, True, (), ("2",), "invalid_horizontal"),
    "cloud_vertical_fallback": ScenarioBlueprint("cloud_vertical_fallback", 3, 4, 2, True, ("6",), ("2",), "cloud_fallback"),
    "timeout_drop_case": ScenarioBlueprint("timeout_drop_case", 6, 6, 6, True, ("6",), ("2",), "deadline_loss"),
...
def _scenario_duration(workload: str) -> int:
    return WORKLOAD_SCENARIO_DURATION[workload]


def _task_count(workload: str) -> int:
    return WORKLOAD_TASK_COUNTS[workload]
...
def _task_deadline(base_duration: int, deadline_pressure: str, arrival_slot: int, workload: str) -> int:
    ...
    workload_offset = _WORKLOAD_DEADLINE_SHIFT[workload]
    return arrival_slot + max(3, base_duration // 2 + pressure_offset + workload_offset)
            ...
            for deadline_pressure in config.deadline_pressures:
                for scenario_name in config.scenarios:
                    ...
                    for index in range(task_count):
                        ...
                        queue_snapshot = _queue_snapshot(index, workload, scenario_name)
