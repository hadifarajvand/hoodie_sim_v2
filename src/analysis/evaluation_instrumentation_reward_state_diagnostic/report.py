...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    sections = [
        ...
        "",
        "## Canonical Task Outcome Summary",
        json_dump(payload["canonical_task_outcome_summary_result"]).strip(),
        "",
        "## Canonical Reward Decomposition",
        json_dump(payload["canonical_reward_decomposition_result"]).strip(),
        "",
        "## Raw vs Canonical Metric Comparison",
        json_dump(payload["raw_vs_canonical_metric_comparison_result"]).strip(),
        "",
        "## Paper-Aligned Diagnostic Metrics",
        json_dump(payload["paper_aligned_diagnostic_metrics_result"]).strip(),
        "",
        ...
        "",
        "## Canonical Policy-Effect Summary",
        json_dump(payload["canonical_policy_effect_summary_result"]).strip(),
        "",
...
def render_final_diagnostic_summary_markdown(payload: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- canonical_task_reward_static_across_budget: `{payload['canonical_task_reward_static_across_budget']}`",
