...
OUTPUT_DIR = Path("artifacts/analysis/paper-default-bounded-pilot")
PILOT_EPISODES = 3
...
def run_bounded_pilot() -> dict[str, Any]:
    ...
    all_episodes_ran = episodes_completed == PILOT_EPISODES
    ...
    report: dict[str, Any] = {
        "pilot_type": "paper-default-bounded-pilot",
...
def write_artifacts(report: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = OUTPUT_DIR / "bounded-pilot-summary.json"
    md_path = OUTPUT_DIR / "bounded-pilot-summary.md"
    ...
    lines = [
        "# Bounded Paper-Default Pilot Campaign",
        "",
        f"- **Verdict**: `{verdict}`",
        ...
        f"- loss_mean: `{m['loss_summary']['mean']}`",
