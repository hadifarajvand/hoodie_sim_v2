...
@contextmanager
def _patched_calibrated_session_environment(session: "StateRepresentationTrainingSession"):
    calibration_root = Path(tempfile.gettempdir()) / session.config.calibration_trace_root_name
...
@contextmanager
def _patched_decision_time_state_injection(trainer: DDQNTrainer):
    original_state_tensor = trainer._state_tensor  # noqa: SLF001
    original_observe_flat = HoodieGymEnvironment.observe_flat

    def _observe_flat_with_context(self, current_task=None):  # type: ignore[no-untyped-def]
        observation = original_observe_flat(self, current_task)
        trainer._decision_time_context = {  # noqa: SLF001
            "observation": dict(observation),
            "current_task": current_task,
            "episode_length": int(trainer.config.evaluation_episode_length),
        }
        return observation

    def _state_tensor_with_context(history):  # type: ignore[no-untyped-def]
        context = getattr(trainer, "_decision_time_context", None)
        if context:
            state_tensor, _, _ = trainer._decision_state_tensor(  # noqa: SLF001
                history=history,
                observation=context["observation"],
                current_task=context["current_task"],
                episode_length=int(context["episode_length"]),
            )
            trainer._decision_time_context = None  # noqa: SLF001
            return state_tensor
        return original_state_tensor(history)

    HoodieGymEnvironment.observe_flat = _observe_flat_with_context  # type: ignore[assignment]
    trainer._state_tensor = _state_tensor_with_context  # type: ignore[assignment]
    try:
        yield
    finally:
        HoodieGymEnvironment.observe_flat = original_observe_flat  # type: ignore[assignment]
        trainer._state_tensor = original_state_tensor  # type: ignore[assignment]
        trainer._decision_time_context = None  # type: ignore[attr-defined]
...
class StateRepresentationTrainingSession:
    ...
    def train_to_budget(self, target_budget: int) -> dict[str, Any]:
        ...
        with _patched_calibrated_session_environment(self):
    ...
    def candidate_policy_result(self, *, checkpoint_budget: int) -> dict[str, Any]:
        with _patched_calibrated_session_environment(self), patched_terminal_evaluator_state_profile(self.campaign_config.state_representation_profile), _patched_decision_time_state_injection(self.trainer):
...
def build_state_representation_policy_effect_comparison(
    *,
    ...
    with _patched_calibrated_session_environment(session), patched_terminal_evaluator_state_profile(session.campaign_config.state_representation_profile), _patched_decision_time_state_injection(session.trainer):
