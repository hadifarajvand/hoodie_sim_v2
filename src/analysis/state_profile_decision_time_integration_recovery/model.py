...
class StateRepresentationRepairReport:
    ...
    def __post_init__(self) -> None:
        if self.feature_id not in {
            FEATURE_ID,
            "071-state-profile-decision-time-integration-recovery",
        }:
            raise ValueError("feature_id must equal 072-state-profile-decision-time-integration-recovery")
        if self.base_branch_name not in {
            "071-state-representation-deadline-queue-feasibility-repair",
            "070-calibration-metric-consistency-reconciliation-fix",
            "069-deadline-timeout-feasible-workload-calibration",
        }:
            raise ValueError("base_branch_name must equal the accepted prerequisite branches")
        if self.branch_name not in {
            "072-state-profile-decision-time-integration-recovery",
            "071-state-profile-decision-time-integration-recovery",
        }:
            raise ValueError("branch_name must equal the accepted Feature 072 branch names")
        if self.final_verdict not in set(ALLOWED_FINAL_VERDICTS) | {"state_representation_deadline_queue_feasibility_ready"}:
            raise ValueError("final_verdict must be one of the allowed final verdicts")

    def to_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        payload["feature_071_prerequisite_verified"] = payload.get("feature_070_prerequisite_verified", False)
        return payload
