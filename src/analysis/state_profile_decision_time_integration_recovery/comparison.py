...
LEGACY_REPORT_PATH = Path("artifacts/analysis/state-representation-deadline-queue-feasibility-repair/state-representation-repair-report.json")
...
def build_action_path_diversity(policy_result: dict[str, Any]) -> dict[str, Any]:
    task_records = dict(policy_result.get("task_records", {}))
    feasibility = build_task_feasibility_summary(task_records, record_sample_limit=25)
    records_by_task_key = dict(feasibility.get("records_by_task_key", {}))
    local_set = {key for key, record in records_by_task_key.items() if bool(record.get("local_feasible_before_deadline", False))}
    horizontal_set = {key for key, record in records_by_task_key.items() if bool(record.get("horizontal_feasible_before_deadline", False))}
    vertical_set = {key for key, record in records_by_task_key.items() if bool(record.get("vertical_feasible_before_deadline", False))}
    count_based = (
        int(feasibility.get("local_feasible_task_count", 0)),
        int(feasibility.get("horizontal_feasible_task_count", 0)),
        int(feasibility.get("vertical_feasible_task_count", 0)),
    )
    set_based = not (local_set == horizontal_set == vertical_set)
    count_based_diversity = len(set(count_based)) > 1
    actions_have_different_feasibility = bool(count_based_diversity or set_based)
    return {
        "feasibility_universe": "U_hypothetical_action_feasibility",
        "count_based_action_feasibility_diversity": count_based_diversity,
        "set_based_action_feasibility_diversity": set_based,
        "actions_have_different_feasibility": actions_have_different_feasibility,
        "local_feasible_task_count": int(feasibility.get("local_feasible_task_count", 0)),
        "horizontal_feasible_task_count": int(feasibility.get("horizontal_feasible_task_count", 0)),
        "vertical_feasible_task_count": int(feasibility.get("vertical_feasible_task_count", 0)),
        "local_feasible_task_ids": sorted(local_set),
        "horizontal_feasible_task_ids": sorted(horizontal_set),
        "vertical_feasible_task_ids": sorted(vertical_set),
        "selected_action_feasible_task_count": int(feasibility.get("selected_action_feasible_task_count", 0)),
        "selected_action_infeasible_task_count": int(feasibility.get("selected_action_infeasible_task_count", 0)),
        "feasible_task_count_universe": "U_selected_action_tasks",
        "selected_action_feasible_task_count_universe": "U_selected_action_tasks",
...
def build_selected_action_feasibility_diagnostics(legacy_policy_100: dict[str, Any], new_policy_100: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "legacy_completed_selected_action_feasible_count": int(legacy_policy_100.get("completed_selected_action_feasible_count", 0)),
        "new_completed_selected_action_feasible_count": int(new_policy_100.get("completed_selected_action_feasible_count", 0)),
        "legacy_dropped_selected_action_feasible_count": int(legacy_policy_100.get("dropped_selected_action_feasible_count", 0)),
        "new_dropped_selected_action_feasible_count": int(new_policy_100.get("dropped_selected_action_feasible_count", 0)),
        "legacy_selected_action_feasible_task_count": int(legacy_policy_100.get("selected_action_feasible_task_count", 0)),
        "new_state_selected_action_feasible_task_count": int(new_policy_100.get("selected_action_feasible_task_count", 0)),
    }
