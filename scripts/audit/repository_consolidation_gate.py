...
LEGACY_ACTIVE_PREFIXES = ("hoodie/", "tests_supported/", "tests_historical/")
TRANSITIONAL_EXACT = {
...
def repository_root() -> Path:
    output = subprocess.run(["git", "rev-parse", "--show-toplevel"], check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout
    return Path(output.strip()).resolve()


def tracked_paths(root: Path) -> tuple[str, ...]:
    payload = _git(root, "ls-files", "-z")
    return tuple(sorted(part.decode("utf-8", errors="surrogateescape") for part in payload.split(b"\0") if part))


def _artifact_issues(paths: Iterable[str]) -> tuple[str, ...]:
    return tuple(path for path in paths if path.startswith("artifacts/") and not path.startswith(ALLOWED_TRACKED_ARTIFACT_PREFIXES))
...
def _transitional_issues(paths: Iterable[str]) -> tuple[str, ...]:
    ...
    for path in paths:
        if path in TRANSITIONAL_EXACT:
            issues.append(path)
        elif path.startswith("src/hoodie/experiments/") and (path.endswith("_patch.py") or path.endswith("_v2.py")):
            issues.append(path)
...
def _packaging_issues(root: Path) -> tuple[str, ...]:
    ...
    if pytest.get("testpaths", []) != expected:
...
def evaluate(root: Path) -> GateResult:
    ...
    passed = not any((artifact_paths, legacy_paths, transitional, legacy_imports, echo_dependencies, packaging, missing))
    return GateResult(artifact_paths, legacy_paths, transitional, legacy_imports, echo_dependencies, packaging, missing, passed)
