...
class HoodieTrainingFoundationReadinessAuditFlowIntegrationTest(unittest.TestCase):
    ...
    def test_tiny_audit_is_deterministic_and_writes_reports(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            runner = HoodieTrainingFoundationReadinessAuditRunner(output_dir=output_dir)
            first = runner.run()
            first_snapshot = {path.name: path.read_text(encoding="utf-8") for path in sorted(output_dir.iterdir()) if path.is_file()}
            second = HoodieTrainingFoundationReadinessAuditRunner(output_dir=output_dir).run()
            ...
            self.assertEqual(first.metadata["feature_id"], "023-training-foundation-readiness-audit")
            self.assertEqual(first.source_gate_status["passed"], True)
            self.assertEqual(first.verdict, "blocked_readiness")
            self.assertEqual(first.reproducibility_details["approved_interpreter"], "/Users/hadi/Documents/GitHub/hoodie_sim_v2/src/.venvmac/bin/python")
    ...
    def test_learned_policy_placeholder_stub_runs_through_shared_environment_interface(self) -> None:
        ...
        while True:
            ...
            if terminated or truncated:
                break
        self.assertIn("finalized_tasks", info)
