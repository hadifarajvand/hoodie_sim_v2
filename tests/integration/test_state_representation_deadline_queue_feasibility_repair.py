...
from src.analysis.state_representation_deadline_queue_feasibility_repair.config import OUTPUT_DIR
from src.analysis.state_representation_deadline_queue_feasibility_repair import runner
from tests.unit.test_state_representation_deadline_queue_feasibility_repair_schema import make_repair_payload
...
def test_runner_creates_report_and_figures(monkeypatch) -> None:
    ...
    monkeypatch.setattr(runner, "git_status_paths", lambda: ["src/analysis/state_representation_deadline_queue_feasibility_repair/runner.py"])
    monkeypatch.setattr(runner, "git_staged_paths", lambda: [])
    monkeypatch.setattr(runner, "git_diff_paths", lambda base_ref: ["src/analysis/state_representation_deadline_queue_feasibility_repair/runner.py"])
    ...
    assert result["final_verdict"] == "state_representation_deadline_queue_feasibility_ready"
    ...
    expected_files = [
        "state-representation-repair-report.json",
        "state-representation-repair-report.md",
