...
class UserApprovedAssumptionPatchRegistryReportIntegrationTest(unittest.TestCase):
    def test_report_has_required_schema_and_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["status_counts"]["approved"], 8)
            self.assertEqual(payload["status_counts"]["proposed"], 0)
            ...
            self.assertEqual([item["item_id"] for item in payload["runtime_usable_items"]], ["Figure_7_adjacency", "legal_horizontal_destinations", "EA_private_cpu_capacity", "EA_public_cpu_capacity", "cloud_cpu_capacity", "cloud_data_rate", "timeout_value", "multi_agent_aggregation_reduction_order"])
            self.assertEqual(payload["blocked_items"], [])
            self.assertEqual(payload["proposed_items"], [])
            ...
            aggregation = next(item for item in payload["entries"] if item["item_id"] == "multi_agent_aggregation_reduction_order")
            ...
            aggregation_payload = aggregation["proposed_value"]
