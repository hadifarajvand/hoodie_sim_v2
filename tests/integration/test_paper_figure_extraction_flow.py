...
class PaperFigureExtractionFlowTests(unittest.TestCase):
    def test_real_paper_baseline_extraction_flow(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(entries["Figure 10"]["support_status"], "supported")
            self.assertTrue(entries["Figure 10"]["extracted_artifact_metrics"]["per_run"])
            self.assertEqual(entries["Figure 9"]["support_status"], "supported")
            ...
            self.assertIn("Do not claim paper reproduction validity from this scaffold.", payload["global_warnings"])
