...
class UserApprovedAssumptionPatchRegistryRuntimeGateTest(unittest.TestCase):
    def test_runtime_use_only_possible_for_approved_assumptions(self) -> None:
        ...
        self.assertEqual([entry.item_id for entry in approved], ["Figure_7_adjacency", "legal_horizontal_destinations", "EA_private_cpu_capacity", "EA_public_cpu_capacity"])
        self.assertTrue(all(entry.runtime_use_allowed for entry in approved))
        self.assertFalse(any(entry.runtime_use_allowed for entry in entries if entry.item_id not in {"Figure_7_adjacency", "legal_horizontal_destinations", "EA_private_cpu_capacity", "EA_public_cpu_capacity"}))
