...
class PaperDefaultPilotTrainingRunMetricsTests(unittest.TestCase):
    def test_pilot_report_requires_growth_and_finite_metrics(self) -> None:
        kwargs = _base_report_kwargs()
        kwargs["replay_summary"]["replay_size"] = 110
        kwargs["replay_summary"]["replay_growth_count"] = 0
        kwargs["replay_summary"]["replay_growth_validated"] = False
        kwargs["optimizer_summary"]["optimizer_step_count"] = 47
        kwargs["optimizer_summary"]["optimizer_step_growth_count"] = 0
        kwargs["optimizer_summary"]["optimizer_progress_validated"] = False
        kwargs["final_verdict"] = "paper_default_pilot_training_passed"
        kwargs["recommended_next_feature"] = "Feature 058 — Evaluation Trace Bank and Baseline Evaluation Harness"
        kwargs["remaining_blockers"] = []
        with self.assertRaises(ValueError):
            PaperDefaultPilotTrainingRunReport(**kwargs)

    def test_pass_path_requires_replay_growth_against_feature_055(self) -> None:
