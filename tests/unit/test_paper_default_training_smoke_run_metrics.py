...
class PaperDefaultTrainingSmokeRunMetricsTests(unittest.TestCase):
    ...
    def test_smoke_run_contracts_are_true_on_the_passing_path(self) -> None:
        ...
        self.assertEqual(payload["replay_summary"]["replay_size"], 0)
        self.assertFalse(payload["replay_summary"]["replay_inserted"])
        self.assertEqual(payload["optimizer_step_summary"]["optimizer_step_count"], 0)
        self.assertFalse(payload["optimizer_step_summary"]["optimizer_steps_executed"])
        self.assertFalse(payload["loss_summary"]["loss_is_finite"])
        self.assertFalse(payload["legal_action_summary"]["legal_action_only"])
        self.assertFalse(payload["delayed_reward_contract_verified"]["delayed_reward_contract_preserved"])
        self.assertFalse(payload["train_eval_contract_verified"]["train_eval_trace_banks_disjoint"])
        self.assertFalse(payload["checkpoint_summary"]["checkpoint_schema_valid"])
