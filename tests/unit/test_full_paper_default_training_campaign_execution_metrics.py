...
class FullPaperDefaultTrainingCampaignExecutionMetricsTests(unittest.TestCase):
    ...
    def test_counts_are_actual_reduced_budget_not_configured_budget_fabrication(self) -> None:
        payload = build_full_paper_default_training_campaign_execution_report().to_dict()
        configured = payload["campaign_execution_summary"]["configured_budget"]["training_episode_count"]
        actual = payload["campaign_execution_summary"]["actual_training_episode_count"]
        self.assertEqual(actual, payload["resource_control_summary"]["actual_executed_budget"]["training_episode_count"])
        self.assertLess(actual, configured)
        self.assertTrue(payload["campaign_execution_summary"]["actual_budget_is_reduced_for_local_validation"])
