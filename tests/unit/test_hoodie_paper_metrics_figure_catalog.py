...
class HoodiePaperMetricsFigureCatalogTests(unittest.TestCase):
    ...
    def test_generate_artifacts_writes_files(self) -> None:
        ...
        self.assertTrue((artifact_dir / "paper_style_plots" / "paper_style_plot_manifest.json").exists())
        self.assertTrue((artifact_dir / "paper_style_plots" / "paper_style_plot_report.md").exists())
        ...
        plot_manifest = json.loads((artifact_dir / "paper_style_plots" / "paper_style_plot_manifest.json").read_text(encoding="utf-8"))
        self.assertEqual(len(plot_manifest["entries"]), 14)
        plot_types = {entry["figure_id"]: entry["plot_type"] for entry in plot_manifest["entries"]}
        self.assertEqual(plot_types["Figure 8a"], "gated_status_plot")
        self.assertEqual(plot_types["Figure 8b"], "gated_status_plot")
        self.assertEqual(plot_types["Figure 11"], "gated_status_plot")
        self.assertEqual(plot_types["Figure 9a"], "approximation_tagged_simulator_output_plot")
        self.assertEqual(plot_types["Figure 9e"], "approximation_tagged_simulator_output_plot")
        self.assertEqual(plot_types["Figure 10a"], "simulator_output_plot")
        self.assertEqual(plot_types["Figure 10f"], "simulator_output_plot")
        self.assertTrue(all(entry["paper_values_used"] is False for entry in plot_manifest["entries"]))
        self.assertTrue(all(entry["output_sync_tuning_performed"] is False for entry in plot_manifest["entries"]))
        self.assertTrue(all(Path(entry["plot_path"]).exists() for entry in plot_manifest["entries"]))
