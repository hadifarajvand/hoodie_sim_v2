...
class PaperDefaultPilotTrainingRunBehaviorEquivalenceTests(unittest.TestCase):
    def test_prerequisite_tag_names_are_unique(self) -> None:
        report = PaperDefaultPilotTrainingRunReport(**_base_report_kwargs())
        names = [entry["name"] for entry in report.prerequisite_tags_verified]
        self.assertEqual(len(names), len(set(names)))

    def test_behavior_safety_fields_cover_no_drift_claims(self) -> None:
        report = PaperDefaultPilotTrainingRunReport(**_base_report_kwargs())
        payload = report.to_dict()
        safety = payload["behavior_safety_summary"]
        for key in (
            "no_full_campaign",
